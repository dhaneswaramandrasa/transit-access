/**
 * Generate kecamatan boundary GeoJSON with kecamatan-level scoring.
 *
 * Uses real administrative boundaries from BIG (Badan Informasi Geospasial) API
 * kecamatan layer. Spatial join: for each BIG boundary polygon, find all H3 hexes
 * that overlap it by checking the hex centroid AND all 6 corner vertices. Checking
 * vertices fixes small/narrow kecamatan where no centroid falls inside.
 *
 * Output: jakarta_kecamatan_boundaries.geojson
 */
...
const outputPath = join(__dirname, "..", "public", "data", "jakarta_kecamatan_boundaries.geojson");

const BIG_KEC_API = "https://geoservices.big.go.id/rbi/rest/services/BATASWILAYAH/Administrasi_AR_Kecamatan_10K/MapServer/0/query";
...
async function fetchArea(whereClause, label) {
  ...
  while (true) {
    ...
    try {
      const res = await fetch(`${BIG_KEC_API}?${params}`, {
...
async function main() {
  ...
  const hexPropsMap = new Map();  // h3_index → feature.properties
  ...
  for (const feature of hexData.features) {
    ...
    hexPropsMap.set(h3, feature.properties);
  }

  // ── Step 2: Fetch BIG kecamatan boundaries ──
  console.log("\nFetching BIG kecamatan boundaries...");
  const bigFeatures = [];
  ...
  console.log(`Total BIG kecamatan features: ${bigFeatures.length}`);

  // ── Step 3: Spatial join — assign each hex to its kecamatan boundary ──
  console.log("\nRunning spatial join (hex centroids → kecamatan polygons)...");
  ...
  for (let bi = 0; bi < bigFeatures.length; bi++) {
    ...
    const kecamatan = titleCase(p.NAMOBJ || "");

    const geom = {
      type: bigFeat.geometry.type,
      coordinates: simplifyGeom(roundCoords(bigFeat.geometry.coordinates), 0.00015),
    };
    ...
    if (matchedH3s.length === 0) {
      outputFeatures.push({
        ...
        properties: { kecamatan, city_code: cityCode, kab_kota: kabKota, hex_count: 0, source: "big" },
      });
    } else {
      const hexes = matchedH3s.map((h3) => hexPropsMap.get(h3));
      outputFeatures.push({
        type: "Feature", geometry: geom,
        properties: { kecamatan, city_code: cityCode, kab_kota: kabKota, hex_count: hexes.length, source: "big", _hexes: hexes },
      });
    }
  }

  // ── Step 5: Kecamatan-level scoring with normalization ──
  console.log("Computing kecamatan-level scores...");

  const featuresWithData = outputFeatures.filter((f) => f.properties._hexes);
  console.log(`  Kecamatan with data: ${featuresWithData.length}`);

  const kecRaw = featuresWithData.map((feat) => {
  ...
  // Normalize across all kecamatan with data (independently from kelurahan/hex normalization)
  const normPop = minMaxNormalize(kecRaw.map((r) => r.pop_total));
  const normDependent = minMaxNormalize(kecRaw.map((r) => r.pct_dependent));
  const normZeroVeh = minMaxNormalize(kecRaw.map((r) => r.pct_zero_vehicle));
  const njopArr = kecRaw.map((r) => r.avg_njop);
  const maxNjop = Math.max(...njopArr);
  const normInvNjop = minMaxNormalize(njopArr.map((v) => maxNjop - v));
  const distArr = kecRaw.map((r) => r.dist_to_transit);
  const maxDist = Math.max(...distArr);
  const normDistInv = minMaxNormalize(distArr.map((v) => maxDist - v));
  const normWalkable = kecRaw.map((r) => r.is_walkable_transit ? 100 : 0);
  const normCapacity = minMaxNormalize(kecRaw.map((r) => r.transit_capacity_weight));
  const normPoiDensity = minMaxNormalize(kecRaw.map((r) => r.local_poi_density));
  const normShedPoi = minMaxNormalize(kecRaw.map((r) => r.transit_shed_poi_count));

  const needScores = kecRaw.map((_, i) =>
    Math.round((0.25 * normPop[i] + 0.25 * normDependent[i] + 0.25 * normZeroVeh[i] + 0.25 * normInvNjop[i]) * 10) / 10
  );
  const accessScores = kecRaw.map((_, i) =>
  ...
  for (const f of outputFeatures) {
    if (f.properties.quadrant) byQuadrant[f.properties.quadrant] = (byQuadrant[f.properties.quadrant] || 0) + 1;
  }
